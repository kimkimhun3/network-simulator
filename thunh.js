...
function sendBufferedPackets(callback) {
  ...
  packetBuffer.forEach(packet => {
      server.send(packet, 0, packet.length, decoderPort, decoderIp, (err) => {
          if (err) {
              console.error(`Error sending packet to decoder: ${err.message}`);
          }
          checkCompletion();
...
function performSingleBuffering() {
  ...
  console.log(`Buffering started at ${new Date(startTime).toISOString()}`);

  bufferTimeout = setTimeout(() => {
      const endTime = Date.now();
      console.log(`Buffering stopped at ${new Date(endTime).toISOString()}`);
      const elapsedTime = endTime - startTime;
      console.log(`Buffering took ${elapsedTime}ms`);
      ...
      bufferTimeout = setTimeout(() => {
          ...
          sendBufferedPackets(() => {
              ...
              if ((enableRepetitionBuffer && waitingTime > 0)  && sendCompleteFlag) {
                  ...
                  console.log(`Starting waiting time`);
                  isWaiting = true;
